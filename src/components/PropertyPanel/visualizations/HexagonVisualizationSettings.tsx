/**
 * HexagonVisualizationSettings - Hexagon-specific visualization settings
 * Single Responsibility: Manages all settings specific to hexagon visualizations
 */
...
export interface HexagonVisualizationSettingsProps {
  settings: {
    // Hexagon-specific
    hexSize: number;
  ...
  onChange: (
    updates: Partial<HexagonVisualizationSettingsProps['settings']>
  ) => void;
  theme: 'frost_light' | 'frost_dark';
  className?: string;
}

export const HexagonVisualizationSettings: React.FC<
  HexagonVisualizationSettingsProps
> = ({ settings, onChange, theme, className = '' }) => {
  ...
  return (
    <div className={`hexagon-visualization-settings ${className}`}>
      {/* Hexagon-specific settings */}
      <div className={sectionClasses}>
        <h4 className={titleClasses}>Hexagon Properties</h4>
        ...
        <SliderControl
          ...
          max={50}
          step={1}
          onChange={value => updateSetting('hexSize', value)}
          theme={theme}
          suffix='px'
          editableValue={true}
        />
...
export default HexagonVisualizationSettings;
