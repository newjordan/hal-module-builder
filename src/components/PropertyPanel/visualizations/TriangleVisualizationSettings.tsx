/**
 * TriangleVisualizationSettings - Triangle-specific visualization settings
 * Single Responsibility: Manages all settings specific to triangle visualizations
 */
...
export interface TriangleVisualizationSettingsProps {
  settings: {
    // Triangle-specific
  ...
  onChange: (
    updates: Partial<TriangleVisualizationSettingsProps['settings']>
  ) => void;
  theme: 'frost_light' | 'frost_dark';
  className?: string;
}

const TRIANGLE_ORIENTATION_OPTIONS: SelectOption[] = [
  { value: 'up', label: 'Up' },
  { value: 'down', label: 'Down' },
  { value: 'alternating', label: 'Alternating' },
];

export const TriangleVisualizationSettings: React.FC<
  TriangleVisualizationSettingsProps
> = ({ settings, onChange, theme, className = '' }) => {
  ...
  return (
    <div className={`triangle-visualization-settings ${className}`}>
      {/* Triangle-specific settings */}
      <div className={sectionClasses}>
        <h4 className={titleClasses}>Triangle Properties</h4>

        <SelectControl
          label='Triangle Orientation'
          value={settings.triangleOrientation}
          options={TRIANGLE_ORIENTATION_OPTIONS}
          onChange={value => updateSetting('triangleOrientation', value as any)}
          theme={theme}
        />
      </div>
...
export default TriangleVisualizationSettings;
